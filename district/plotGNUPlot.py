...
def PlotAll(C, A, assignment, bounded_regions, bbox, output):
    ...
    for i in range(len(C)):
        f.write(str(C[i][0])+" "+str(C[i][1])+" "+str(colors[i%63])+"\n")
    for j in range(len(A)):
        f.write(str(A[j][0])+" "+str(A[j][1])+" "+str(colors[assignment[j]%63])+"\n")
...
def Parse_boundary(filename):
    f = open(filename, "r")
    f = open('/Users/jay/Desktop/Sem6/Machine_Learning/Project/district/temp_voronoi_boundary_bs_tester2', "r")
    ...
    for l in lines:
        ...
        y = float(s[1])
        points.append([x,y])
    to_add = list(Polygon(points).buffer(0))
    for a in to_add:                  
        boundaries.append(a)
    f.close()
    return boundaries
...
def GNUplot_boundary(p,f):
    ...
    # print("________")
    invalid = []
    i = invalid[0].exterior
    invalid[0].is_valid
    ff = [x for x in boundary if x not in invalid]
    ff[0].exterior
    for p in boundary:
        try:
            x,y = p.exterior.xy
...
if __name__ == '__main__':
    ...
    boundary = Parse_boundary('/Users/jay/Desktop/Sem6/Machine_Learning/Project/district/temp_state_boundary_bs_tester2')
